...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`population\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`population_updated_at_idx\` ON \`population\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`population_created_at_idx\` ON \`population\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`projects_villages\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`projects_villages_order_idx\` ON \`projects_villages\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`projects_villages_parent_idx\` ON \`projects_villages\` (\`parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`initiatives_villages\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`initiatives_villages_order_idx\` ON \`initiatives_villages\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`initiatives_villages_parent_idx\` ON \`initiatives_villages\` (\`parent_id\`);`)
  await db.run(sql`CREATE TABLE \`initiatives\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`category\` text NOT NULL,
  	\`status\` text DEFAULT 'active' NOT NULL,
  	\`beneficiaries\` numeric DEFAULT 0,
  	\`start_date\` text,
  	\`end_date\` text,
  	\`description\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`programs\` (
  	...
  	\`name\` text NOT NULL,
  	\`category\` text NOT NULL,
  	\`stage\` text DEFAULT 'concept' NOT NULL,
  	\`participants\` numeric DEFAULT 0,
  	\`target_participants\` numeric,
  	\`funding_source\` text,
  	\`funding_amount\` text,
  	\`description\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`members\` (
  	...
  	\`phone\` text,
  	\`village\` text NOT NULL,
  	\`membership_type\` text DEFAULT 'ordinary',
  ...
  await db.run(sql`CREATE TABLE \`meeting_minutes\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`meeting_type\` text,
  	\`date\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`meeting_minutes_attachment_idx\` ON \`meeting_minutes\` (\`attachment_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`financials\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`type\` text NOT NULL,
  	\`category\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`financials_attachment_idx\` ON \`financials\` (\`attachment_id\`);`)
  await db.run(sql`CREATE INDEX \`financials_updated_at_idx\` ON \`financials\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`financials_created_at_idx\` ON \`financials\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`announcements_target_villages\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`announcements\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`announcements_target_villages_order_idx\` ON \`announcements_target_villages\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`announcements_target_villages_parent_idx\` ON \`announcements_target_villages\` (\`parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`population_id\`) REFERENCES \`population\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	...
  	FOREIGN KEY (\`financials_id\`) REFERENCES \`financials\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_population_id_idx\` ON \`payload_locked_documents_rels\` (\`population_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_financials_id_idx\` ON \`payload_locked_documents_rels\` (\`financials_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`population\`;`)
  await db.run(sql`DROP TABLE \`projects_villages\`;`)
  await db.run(sql`DROP TABLE \`projects\`;`)
  await db.run(sql`DROP TABLE \`initiatives_villages\`;`)
  ...
  await db.run(sql`DROP TABLE \`financials\`;`)
  await db.run(sql`DROP TABLE \`announcements_target_villages\`;`)
